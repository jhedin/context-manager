...
5. Execute the chosen action for each topic ID:
   - **SUMMARIZE**:
     - Call `summarize_topic` with just the topic_id — if a `[READY]` dormant summary exists it activates instantly; otherwise `claude-opus-4-6` auto-generates the summary via API. No `get_topic_content` call needed.
   - **BYPASS**: Call `bypass_topic`.
   - **RESTORE**: The user wants to restore the *original content* that was replaced by a summary or bypass. If the selected topic is a summary entry, you need to find and restore the **original orphaned topic** it replaced — look in `list_topics` output for an orphaned topic whose content matches what the summary describes. Call `restore_topic` with the *original* topic's ID, not the summary's ID. The restore operation will re-insert the original messages and orphan the summary.
6. **Pre-generate dormant summaries for remaining topics.** After completing the user's chosen action, look at the remaining active topics that don't have `[READY]` tags. For each one, call `prepare_summary` with just the topic_id — `claude-opus-4-6` auto-generates the summary via API. This ensures future `/prune` calls can activate summaries instantly. Do this in the background if possible.
...
Summaries are now auto-generated by `claude-opus-4-6` — you do not need to write them manually. If you pass a `summary` parameter to `summarize_topic` or `prepare_summary`, it overrides auto-generation.
